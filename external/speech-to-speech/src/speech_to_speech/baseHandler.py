...
from speech_to_speech.pipeline.control import PipelineControlMessage, is_control_message, END_OF_TURN, SESSION_END
...
class BaseHandler(Generic[InT, OutT]):
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            if isinstance(item, PipelineControlMessage) and is_control_message(item, END_OF_TURN.kind):
                logger.debug(f"{self.__class__.__name__}: end of turn received")
                try:
                    for output in self.on_end_of_turn():
                        self.before_emit_output(output)
                        self.queue_out.put(cast(OutT | PipelineControlMessage | bytes, output))
                except Exception as e:
                    logger.error(
                        f"{self.__class__.__name__}: Error in on_end_of_turn(): {type(e).__name__}: {e}",
                        exc_info=True,
                    )
                # Not forwarded: it addresses whoever owns the client's queue (the
                # VAD), and what it produces there is a normal turn. Downstream
                # handlers see that turn, not the fact that a client asked for it.
                continue
    ...
    def on_end_of_turn(self) -> Iterator[OutT]:
        """Close whatever turn is open right now, because the client says it is.

        Only a handler that buffers a turn has anything to close — for everyone
        else this is correctly a no-op. Yielded outputs go out the normal way, so
        a forced turn is indistinguishable downstream from a detected one.
        """
        return iter(())
