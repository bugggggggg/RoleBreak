...
def build_pipeline(
    ...
    # Build the shared RuntimeConfig up front (non-realtime modes) so the
    # websocket comms handler can mutate its persona (session.instructions) from
    # a client-sent text_prompt before the conversation starts. The LLM handler
    # re-reads session.instructions every turn, so a late update still takes.
    if module_kwargs.llm_backend in ("responses-api", "chat-completions"):
