...
class BaseOpenAICompatibleHandler(BaseHandler[LLMIn, LLMOut], ABC):
    ...
    def setup(
        ...
        **_kwargs: Any,
        ...
        # Uncapped, the server generates until the context window is full. A model
        # that falls into a repetition loop then returns a reply thousands of tokens
        # long, which is written back into ``Chat`` -- so every later turn of that
        # conversation is rejected for overflowing the context. The cap bounds both.
