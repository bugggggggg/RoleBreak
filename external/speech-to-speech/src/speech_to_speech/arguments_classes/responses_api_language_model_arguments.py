...
class ResponsesApiLanguageModelHandlerArguments(LanguageModelBaseArguments):
    ...
    responses_api_max_output_tokens: Optional[int] = field(
        default=None,
        metadata={
            "help": "Hard cap on the tokens one reply may generate, sent as max_tokens / max_output_tokens on "
            "every request. Without it the server is free to generate until the model's context is full, so a "
            "single degenerate reply (a repetition loop) both takes minutes to synthesize and lands in the chat "
            "history, after which every later turn of that conversation overflows the context window and fails. "
