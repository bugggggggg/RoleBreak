...
class VADHandlerArguments:
    ...
    manual_turn_end: bool = field(
        default=False,
        metadata={
            "help": "Close each turn only when the client sends {\"type\": \"end_of_turn\"}, never on detected silence. For callers that know where the turn ends rather than having to guess -- a benchmark replaying a fixed clip, or push-to-talk -- so an in-sentence pause cannot cut the utterance short. min_silence_ms is then unused, and turn_detection.silence_duration_ms from a realtime client is ignored. Default is False (silence detects the boundary)."
        },
    )
