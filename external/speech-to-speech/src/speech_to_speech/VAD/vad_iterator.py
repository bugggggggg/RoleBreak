...
class VADIterator:
    ...
    def _close_utterance(self) -> list[torch.Tensor]:
        """Hand back the buffered utterance and arm for the next one."""
        self.temp_end = 0
        self.triggered = False
    ...
    def flush(self) -> list[torch.Tensor] | None:
        """Close the current utterance now, the way end-of-speech would.

        ``min_silence_duration_ms`` is a *guess* at where the speaker stopped, and
        it is wrong in both directions: it cuts in on a thinking pause and it
        waits out a speaker who has finished. A caller that does not have to
        guess — a benchmark replaying a clip of known length, a push-to-talk UI
        with a released button — calls this instead and gets the real boundary.

        Returns ``None`` when no utterance is open, so a client that commits a
        turn twice, or commits during silence, is a no-op rather than an empty
        turn.
        """
