...
# Silence bar used in manual-turn mode, in seconds of audio: an hour, so the
# comparison in ``VADIterator.__call__`` can never be satisfied by a real session
# and only an explicit flush closes a turn. A plain sentinel would have to be
# special-cased in the iterator's arithmetic; a span that long simply never wins.
_NEVER_ENDPOINT_S = 3600
...
class VADHandler(BaseHandler[VADIn, VADOut]):
    ...
    def setup(
        ...
        self.manual_turn_end = manual_turn_end
        if manual_turn_end:
            # The client commits every turn with END_OF_TURN, so the silence
            # heuristic must never beat it to the punch — otherwise the first
            # in-sentence pause still ends the turn and the commit arrives to an
            # already-answered prefix. Pushing the silence bar out of reach is
            # the whole switch: ``VADIterator`` keeps appending to its buffer
            # (pauses included, so the utterance stays intact) and only
            # :meth:`VADIterator.flush` ever closes it.
            self.iterator.min_silence_samples = sample_rate * _NEVER_ENDPOINT_S
            logger.info("VAD: manual turn end — turns close on client END_OF_TURN, not on silence")
    ...
    def _apply_runtime_turn_detection(self, runtime_config: RuntimeConfig | None = None) -> None:
        ...
        if "silence_duration_ms" in td:
            if self.manual_turn_end:
                # Manual turns own the boundary outright: honouring this would
                # hand endpointing back to the silence heuristic and reintroduce
                # exactly the mid-utterance cut the mode exists to prevent.
                logger.info(
                    "VAD: ignoring turn_detection.silence_duration_ms=%sms (manual turn end)",
    ...
    def on_end_of_turn(self) -> Iterator[VADOut]:
        """Close the buffered turn because the client says the user has stopped.

        Runs the flushed utterance through the very same finalization path a
        silence-detected turn takes, so nothing downstream can tell the
        difference — same events, same speculative-turn bookkeeping, same
        ``VADAudio(mode="final")``. The progressive-release block inside
        :meth:`_process_realtime` no-ops on the way through because
        :meth:`VADIterator.flush` has already emptied the buffer it reads.
        """
        if not self.should_listen.is_set():
            # Mid-reply: the server is talking and this turn's audio was never
            # taken in. Committing here would emit whatever the last turn left.
            logger.debug("VAD: end_of_turn ignored (not listening)")
            return
        flushed = self.iterator.flush()
        if flushed is None:
            logger.debug("VAD: end_of_turn with no speech buffered")
            return
        # The duration is not recomputed here: the finalization path below logs
        # the segment it actually closed ("Speech soft-ended ..."), and it knows
        # about the speculative prefix this method cannot see.
        logger.info("VAD: end_of_turn committed by client (%d chunk(s))", len(flushed))
        if self._uses_realtime_turn_handling():
            yield from self._process_realtime(flushed)
        else:
            yield from self._process_normal(flushed)
