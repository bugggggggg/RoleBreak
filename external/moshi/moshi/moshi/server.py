...
class ServerState:
    ...
    async def decode_and_send(
        ...
        # sphn >= 0.2.1 returns the Opus bytes directly from append_pcm()
        # (the old read_bytes() split call was removed).
    ...
    async def recv_loop(
        ...
        try:
            async for message in ws:
                ...
                if kind == 1:  # audio
                    payload = message[1:]
                    # sphn >= 0.2.1 returns decoded PCM directly from append_bytes()
                    # (the old separate read_pcm() call was removed).
