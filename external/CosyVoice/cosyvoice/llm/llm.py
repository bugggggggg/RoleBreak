...
class Qwen2Encoder(torch.nn.Module):
    def __init__(self, pretrain_path):
        super().__init__()
        # The checkpoint config pins torch_dtype=bfloat16; modern transformers
        # honours that on load, but the rest of CosyVoice (speech_embedding, flow,
        # hift) runs in float32 and feeds float32 inputs_embeds in here — so force
        # the LLM to float32 too, else q_proj hits "mat1 and mat2 must have the
        # same dtype" (Float vs BFloat16). fp16/bf16 is opt-in via autocast upstream.
    ...
    def forward_one_step(self, xs, masks, cache=None):
        # NOTE single unpadded causal sequence: pass attention_mask=None and let
        # transformers build the mask from cache_position. The old masks[:, -1, :]
        # yields a length-1 mask once decoding starts, which on transformers>=5
        # (reworked Cache/cache_position masking) stops the new token attending
        # over the KV cache -> wrong speech tokens -> garbled audio.
        outs = self.model(
            inputs_embeds=xs,
            attention_mask=None,
            output_hidden_states=True,
            ...
            if prompt_speech_token_emb.size(1) == 0:
                if (len(out_tokens) != 0 and out_tokens[-1] == self.fill_token) or (len(out_tokens) == 0 and lm_input.size(1) == 1):
                    ...
                    if text_cache.size(1) >= self.mix_ratio[0]:
                        ...
                        continue
                while True:
                    seq_len = lm_input.shape[1] if cache is None else lm_input.shape[1] + cache.get_seq_length()
                    y_pred, cache = self.llm.forward_one_step(lm_input,
