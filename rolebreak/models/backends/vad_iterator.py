"""Streaming Silero-VAD iterator.

Copied verbatim from ``external/speech-to-speech/src/speech_to_speech/VAD/
vad_iterator.py`` so the package has no dependency on that repo being on the
import path. Used by :mod:`rolebreak.models.backends.moshi_api` to detect when
Moshi has finished a turn during a multi-turn duplex conversation.
"""
