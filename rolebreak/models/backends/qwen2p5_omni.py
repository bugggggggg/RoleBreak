"""Qwen2.5-Omni (dense 7B) speech-to-speech backend, served over an OpenAI-compatible API.

``Qwen/Qwen2.5-Omni-7B`` perceives audio (also image/video) and emits both text
and speech. As with Qwen3-Omni we drive it out-of-process: run the checkpoint
behind ``vllm serve --omni`` and point this backend at that endpoint. The
shipped voices are Chelsie / Ethan, default ``Chelsie``.

Deliberately a sibling of :mod:`rolebreak.models.backends.qwen3_omni` rather
than a subclass — the two servers disagree on where the audio comes back (see
:class:`Qwen2p5OmniAPIModel`), so sharing an implementation would mean a base
class that is mostly overridden.
"""
...
DEFAULT_VOICE = "Chelsie"  # the other shipped voice is "Ethan"

# A locally deployed `vllm serve --omni` OpenAI-compatible endpoint for
# Qwen2.5-Omni. Distinct from the Qwen3-Omni port so both can run side by side.
DEFAULT_API_BASE_URL = "http://localhost:8091/v1"

logger = logging.getLogger(__name__)

DEFAULT_MAX_RETRIES = 3
# A cold three-stage Omni server needs minutes to come back: it reloads the
# weights, recompiles, and recaptures CUDA graphs per stage. 7B dense is
# quicker to reload than Qwen3-Omni's 30B MoE, but waiting is the whole point
# of the retry, so the budget stays generous.
...
class Qwen2p5OmniAPIModel(SpeechToSpeechModel):
    """Qwen2.5-Omni via a locally deployed vLLM OpenAI-compatible endpoint.
    ...
    Two quirks of the ``vllm-omni`` build shape this backend:

    * the talker reads its voice from a **top-level ``speaker``** field, not from
      OpenAI's ``audio.voice`` — so we mirror the voice there;
    * unlike Qwen3-Omni, the **non-streaming** call does return speech, but it
      splits the reply across **two ``choices`` that both carry ``index: 0``** —
      one holds ``message.content`` (text, with ``audio: null``), the other
      holds ``message.audio`` (base64 WAV, mono 24 kHz, with ``content: null``
      and an empty ``transcript``). Reading ``choices[0]`` the usual way would
      silently drop the speech, so :meth:`_parse_completion` folds every choice
      into one reply and backfills the transcript from the text choice.

    Non-streaming is therefore the default path. Pass ``stream=True`` to use the
    SSE path instead: it carries the same content (text deltas, then the WAV as
    one base64 segment in ``delta.content``, flagged by a re-announced
    ``role: assistant``) and additionally yields a time-to-first-token latency.
    """
    ...
    def generate(self, request: ChatRequest) -> ChatResponse:
        ...
        # Default the voice so spoken replies use a shipped Qwen2.5-Omni speaker.
        audio = payload.setdefault("audio", {})
        audio.setdefault("voice", DEFAULT_VOICE)

        if "audio" in payload.get("modalities", []):
            # The talker reads its voice from a top-level ``speaker`` field.
    ...
    def _generate_stream(self, payload: dict[str, Any]) -> ChatResponse:
        ...
        a turn that produced only speech, the talker's first WAV segment. (Unlike
        Qwen3-Omni this server emits the speech as a single trailing segment, so
        first-audio here is effectively end-of-generation, not a start-of-speech
        latency.)
        """
        text_parts: list[str] = []
